...
- **ML Frameworks**: Unified Model-Agnostic LLM Client (`config.llm_client`), Google Cloud Vertex AI ADC (`gemini-2.5-flash`, `text-embedding-004` 768-dim), OpenAI SDK (`gpt-5`, `gpt-5-mini`), `FlashRank` (`ms-marco-MiniLM-L-12-v2` cross-encoder ONNX CPU)
- **Vector Search**: `Qdrant` (Dense HNSW vectors with 768-dim and 1536-dim support), `rank-bm25` (Sparse with financial regex tokenization)
- **Agentic Routing & Verification**: `Pydantic` + Native JSON Schema Structured Outputs (Cross-Provider)
- **Infrastructure**: `FastAPI`, `Streamlit`, `Docker Compose`
- **Observability**: `OpenTelemetry` (OTLP distributed tracing), `Jaeger`, `Prometheus`, `Grafana`, `OnlineQualityMonitor` (Live SLI Auditing)
- **Code Quality**: Strict `mypy` typing, `ruff` checks, `bandit` security scanning, `pytest` suite (890+ unit and integration tests with 100% pass rate).
...
## 🧪 Testing & CI/CD
This repository boasts a robust testing apparatus with **898 unit and integration tests** passing across 49 test suites with **high code coverage (80%)**.
```bash
...
| **[Design Decisions (ADRs)](docs/DESIGN_DECISIONS.md)** | 22 Architectural Decision Records detailing production trade-offs (BM25, FlashRank, valley ordering, UUID5, semantic cache threshold, concurrency semaphores, sub-query decomposition, contextual compression, calibrated abstention, and online SLI monitoring). |
...
| **[API Reference](docs/API_REFERENCE.md)** | OpenAPI / Swagger schema specifications, typed SSE frames, and health check contracts. |
| **[CI/CD & Automation](docs/CI_CD.md)** | GitHub Actions workflow matrix (ruff, mypy, bandit, trufflehog, pytest). |
