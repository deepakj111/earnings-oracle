...
class TestAllRetrievalQueries:
    ...
    def test_stepback_query_included(self) -> None:
    ...
    def test_multi_queries_appear_before_stepback(self) -> None:
    ...
    def test_whitespace_stripped_from_result(self) -> None:
        tq = _make(multi_queries=["  q1  "], stepback_query="  q2  ")
        result = tq.all_retrieval_queries
        assert "q1" in result
        assert "q2" in result

    def test_four_unique_queries_all_present(self) -> None:
        tq = _make(multi_queries=["q1", "q2", "q3"], stepback_query="q4")
        assert len(tq.all_retrieval_queries) == 4

    def test_stepback_not_duplicated_when_matches_multi(self) -> None:
        tq = _make(
