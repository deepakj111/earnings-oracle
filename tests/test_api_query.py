...
class TestCacheInvalidate:
    def test_invalidate_with_ticker(self, client: TestClient, mock_pipeline: MagicMock) -> None:
        resp = client.post("/query/cache/invalidate?ticker=NVDA")
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["ticker"] == "NVDA"
        mock_pipeline._cache.invalidate_ticker.assert_awaited_with("NVDA")

    def test_invalidate_all_entries(self, client: TestClient, mock_pipeline: MagicMock) -> None:
        resp = client.post("/query/cache/invalidate")
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert "flushed completely" in data["message"]
        mock_pipeline._cache.client.delete_collection.assert_awaited_with("semantic_cache")

    def test_invalidate_when_cache_not_configured(
        self, client: TestClient, mock_pipeline: MagicMock
    ) -> None:
        mock_pipeline._cache = None
        resp = client.post("/query/cache/invalidate")
        assert resp.status_code == 200
