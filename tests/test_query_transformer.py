...
def _mock_openai_client(content: str = "mocked response text") -> MagicMock:
    ...
    mock_client.chat.completions.create = AsyncMock(return_value=_make_openai_response(content))
    return mock_client
...
class TestRunHyde:
    ...
    async def test_returns_llm_output(self) -> None:
        with patch(
            "query.transformer._call_llm",
            new_callable=AsyncMock,
            return_value="hypothetical passage",
        ):
            assert await _run_hyde("some query") == "hypothetical passage"

    @pytest.mark.asyncio
    async def test_passes_hyde_system_prompt(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "passage"
            await _run_hyde("some query")
        assert mock_llm.call_args.kwargs["system"] == HYDE_SYSTEM

    @pytest.mark.asyncio
    async def test_user_prompt_contains_query(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "passage"
            await _run_hyde("Apple Q4 2024 revenue?")
        assert "Apple Q4 2024 revenue?" in mock_llm.call_args.kwargs["user"]

    @pytest.mark.asyncio
    async def test_uses_hyde_temperature(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "passage"
            await _run_hyde("some query")
    ...
    @pytest.mark.asyncio
    async def test_label_is_hyde(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "passage"
            await _run_hyde("query")
...
class TestRunStepback:
    ...
    async def test_returns_llm_output(self) -> None:
        with patch(
            "query.transformer._call_llm", new_callable=AsyncMock, return_value="broader question"
        ):
            assert await _run_stepback("specific metric question") == "broader question"

    @pytest.mark.asyncio
    async def test_user_prompt_contains_query(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "broader"
            await _run_stepback("specific Q4 AAPL metric")
        assert "specific Q4 AAPL metric" in mock_llm.call_args.kwargs["user"]

    @pytest.mark.asyncio
    async def test_uses_stepback_temperature(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "broader"
            await _run_stepback("query")
        assert mock_llm.call_args.kwargs["temperature"] == _TEMP_STEPBACK

    @pytest.mark.asyncio
    async def test_passes_stepback_system_prompt(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "broader"
            await _run_stepback("query")
...
class TestRunMultiQuery:
    @pytest.mark.asyncio
    async def test_original_is_always_first(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = (
                "Rephrasing one sentence\nRephrasing two sentence\nRephrasing three"
            )
            result = await _run_multi_query("original query here")
    ...
    async def test_returns_list_of_strings(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            ...
            result = await _run_multi_query("Apple Q4 revenue")
        assert isinstance(result, list)
        assert all(isinstance(q, str) for q in result)

    @pytest.mark.asyncio
    async def test_strips_numbering_from_rephrasings(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = (
            ...
            result = await _run_multi_query("Apple revenue")
        for q in result[1:]:
            assert not q[0].isdigit()

    @pytest.mark.asyncio
    async def test_filters_lines_fewer_than_three_words(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "yes\nA proper rephrasing of the original query\nno"
            result = await _run_multi_query("test query for filtering")
        for q in result[1:]:
            assert len(q.split()) >= 3

    @pytest.mark.asyncio
    async def test_deduplicates_rephrasings(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "Apple revenue Q4\nApple revenue Q4\nApple revenue Q4"
            result = await _run_multi_query("Apple revenue Q4")
        lower = [q.lower() for q in result]
        assert len(lower) == len(set(lower))

    @pytest.mark.asyncio
    async def test_max_four_queries_returned(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = (
            ...
            result = await _run_multi_query("original query")
        assert len(result) <= 4

    @pytest.mark.asyncio
    async def test_uses_multi_query_temperature(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "Rephrasing one full sentence"
            await _run_multi_query("Apple Q4 revenue")
        assert mock_llm.call_args.kwargs["temperature"] == _TEMP_MULTI

    @pytest.mark.asyncio
    async def test_empty_lines_excluded_from_result(self) -> None:
        with patch("query.transformer._call_llm", new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "\nWhat was Apple revenue?\n\nHow did Apple perform?\n"
            result = await _run_multi_query("Apple Q4")
...
class TestCallLlm:
    ...
    async def test_returns_stripped_text_on_success(self) -> None:
        ...
        with patch("query.transformer.get_async_openai_client", return_value=mock_client):
    ...
    async def test_max_tokens_passed_to_client(self) -> None:
        ...
        with patch("query.transformer.get_async_openai_client", return_value=mock_client):
            ...
            await _call_llm("sys", "usr", 0.3, 999, "test")
        assert mock_client.chat.completions.create.call_args.kwargs["max_completion_tokens"] == 999

    @pytest.mark.asyncio
    async def test_system_and_user_messages_structured_correctly(self) -> None:
        mock_client = _mock_openai_client("ok")
        with patch("query.transformer.get_async_openai_client", return_value=mock_client):
            from query.transformer import _call_llm

            await _call_llm("MY SYSTEM PROMPT", "MY USER MSG", 0.3, 100, "test")
        messages = mock_client.chat.completions.create.call_args.kwargs["messages"]

        # Verify separate system and user message roles (ADR-013)
    ...
    @pytest.mark.asyncio
    async def test_empty_response_raises_exception(self) -> None:
...
class TestQueryTransformerTransform:
    ...
    async def _patched_transform(self, query, *, hyde="hyde doc", multi=None, stepback="broader q"):
        if multi is None:
            multi = [query, "rephrasing one", "rephrasing two"]
        t = QueryTransformer(enable_cache=False)
        with (
            patch("query.transformer._run_hyde", new_callable=AsyncMock, return_value=hyde),
            patch("query.transformer._run_multi_query", new_callable=AsyncMock, return_value=multi),
            patch("query.transformer._run_stepback", new_callable=AsyncMock, return_value=stepback),
        ):
            return await t.transform(query)

    @pytest.mark.asyncio
    async def test_returns_transformed_query_instance(self) -> None:
        result = await self._patched_transform("Apple Q4 revenue?")
        assert isinstance(result, TransformedQuery)

    @pytest.mark.asyncio
    async def test_original_query_preserved_in_result(self) -> None:
        result = await self._patched_transform("Apple Q4 revenue?")
        assert result.original == "Apple Q4 revenue?"

    @pytest.mark.asyncio
    async def test_hyde_document_populated(self) -> None:
        result = await self._patched_transform("Apple revenue?", hyde="Apple earned $94.9B")
        assert result.hyde_document == "Apple earned $94.9B"

    @pytest.mark.asyncio
    async def test_multi_queries_populated(self) -> None:
        result = await self._patched_transform("q?", multi=["q?", "alt1", "alt2"])
        assert result.multi_queries == ["q?", "alt1", "alt2"]

    @pytest.mark.asyncio
    async def test_stepback_query_populated(self) -> None:
        result = await self._patched_transform("q?", stepback="broader abstract q")
    ...
    @pytest.mark.asyncio
    async def test_query_stripped_before_processing(self) -> None:
        result = await self._patched_transform("  Apple revenue?  ")
        assert result.original == "Apple revenue?"

    @pytest.mark.asyncio
    async def test_failed_techniques_empty_on_full_success(self) -> None:
        result = await self._patched_transform("test query")
        assert result.failed_techniques == []

    @pytest.mark.asyncio
    async def test_graceful_degradation_when_hyde_fails(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch(
                "query.transformer._run_hyde",
                new_callable=AsyncMock,
            ...
            patch(
                "query.transformer._run_stepback", new_callable=AsyncMock, return_value="broader"
            ),
        ):
            result = await t.transform("Apple revenue?")
        assert isinstance(result, TransformedQuery)
        assert "hyde" in result.failed_techniques

    @pytest.mark.asyncio
    async def test_graceful_degradation_when_stepback_fails(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch("query.transformer._run_hyde", new_callable=AsyncMock, return_value="hyde"),
            patch("query.transformer._run_multi_query", new_callable=AsyncMock, return_value=["q"]),
            patch(
            ...
            result = await t.transform("Apple revenue Q4?")
        assert isinstance(result, TransformedQuery)
        assert "stepback" in result.failed_techniques

    @pytest.mark.asyncio
    async def test_graceful_degradation_when_multi_query_fails(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch("query.transformer._run_hyde", new_callable=AsyncMock, return_value="hyde"),
            patch(
    ...
    @pytest.mark.asyncio
    async def test_hyde_fallback_is_original_query(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch(
                "query.transformer._run_hyde",
            ...
            patch("query.transformer._run_multi_query", new_callable=AsyncMock, return_value=["q"]),
            patch("query.transformer._run_stepback", new_callable=AsyncMock, return_value="s"),
        ):
            result = await t.transform("Apple revenue Q4?")
        assert result.hyde_document == "Apple revenue Q4?"

    @pytest.mark.asyncio
    async def test_stepback_fallback_is_original_query(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch("query.transformer._run_hyde", new_callable=AsyncMock, return_value="hyde"),
            patch("query.transformer._run_multi_query", new_callable=AsyncMock, return_value=["q"]),
    ...
    @pytest.mark.asyncio
    async def test_multi_fallback_is_list_with_original(self) -> None:
        t = QueryTransformer(enable_cache=False)
        with (
            patch("query.transformer._run_hyde", new_callable=AsyncMock, return_value="hyde"),
            patch(
    ...
    async def test_cache_hit_returns_same_object(self) -> None:
        ...
        with (
            ...
            r1 = await t.transform("Apple revenue?")
            r2 = await t.transform("Apple revenue?")
        assert r1 is r2

    @pytest.mark.asyncio
    async def test_cache_disabled_calls_llm_each_time(self) -> None:
        t = QueryTransformer(enable_cache=False)
        call_count = {"n": 0}

        async def counting_hyde(q):
            call_count["n"] += 1
            return "hyde"
        ...
        with (
            ...
            await t.transform("Apple revenue?")
            await t.transform("Apple revenue?")
    ...
    async def test_cache_is_case_insensitive(self) -> None:
        ...
        with (
            ...
            r1 = await t.transform("Apple Revenue Q4?")
            r2 = await t.transform("apple revenue q4?")
        assert r1 is r2

    @pytest.mark.asyncio
    async def test_all_three_techniques_invoked(self) -> None:
        t = QueryTransformer(enable_cache=False)
        calls = set()

        async def fake_hyde(q):
            calls.add("hyde")
            return "h"
        ...
        async def fake_stepback(q):
            calls.add("stepback")
            return "s"

        with (
            patch("query.transformer._run_hyde", side_effect=fake_hyde),
            patch("query.transformer._run_multi_query", side_effect=fake_multi),
            patch("query.transformer._run_stepback", side_effect=fake_stepback),
        ):
            await t.transform("Apple revenue?")
